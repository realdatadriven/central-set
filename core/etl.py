    ...
    async def transform(self):
        ...
        try:
            ...
            if not _read.get('success'): # IN CASE OF ERR
                ...
                if _conf.get('params'):
                    _database = copy.deepcopy(_conf.get('params'))#etl_report_base_conf
        ...
        try:
            ...
            if not _read.get('success'): # IN CASE OF ERR
                ...
                for _dt in dates:
                    ...
                    if _conf.get('duckdb_style') is True:
    ...
    async def _duckdb(self, _input, _etlrb, _conf, _conf_etlrb):
        ...
        try:
            ...
            for patt in file_ref_patts:
                ...
                if len(match) > 0:
                    try:
                        file_ref = datetime.datetime.strptime(str(match[0]), patt.get('fmrt'))
                    except Exception as _err:# pylint: disable=broad-exception-caught
                        print(str(_err))
                    break
            ...
            if not _conf['duckdb'].get('pragmas_config_sql'):
                pass
            elif isinstance(_conf['duckdb'].get('pragmas_config_sql'), list):
                for pragmas_config_sql in _conf['duckdb'].get('pragmas_config_sql'):
                    try:
                        pragmas_config_sql = self.set_str_env(pragmas_config_sql)
                        conn.sql(pragmas_config_sql)
                    except Exception as _err:
                        conn.close()
                        *_, exc_tb = sys.exc_info()
                        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                        print('DEBUG INF DUCKDB PRAGMA / CONFIG: ', str(_err), fname, exc_tb.tb_lineno)
                        return {'success': False, 'msg': self.i18n('duckdb-pragma-conf-faild', pconf = pragmas_config_sql, err = str(_err))}
            ...
            if len(_match_insert_patt) > 0:
                ...
                _match_select_star = []
                if len(_sql_aux) > 0:
                    patt = r'SELECT.+\*.+FROM\s+.?.?\w+'
                    _match_select_star = re.findall(patt, _sql_aux[0])
                    # print('_match_select_star:', _match_select_star)
